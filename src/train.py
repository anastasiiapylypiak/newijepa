...
def main(args, resume_preempt=False):
    ...
    batch_size = args['data']['batch_size']
    pin_mem = args['data']['pin_mem']
    num_workers = min(args['data']['num_workers'], 8)  # Reduce to 8 or less
    root_path = args['data']['root_path']
    image_folder = args['data']['image_folder']
    crop_size = args['data']['crop_size']
    crop_scale = args['data']['crop_scale']
    ...
    optimizer, scaler, scheduler, wd_scheduler = init_opt(
        ...
        num_epochs=num_epochs,
        ipe_scale=ipe_scale,
        use_bfloat16=use_bfloat16 and torch.cuda.is_available())  # Use bfloat16 only if CUDA is available
    ...
    # Ensure proper cleanup of DataLoader workers
    unsupervised_loader.dataset.cleanup()
